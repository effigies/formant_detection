...
form Input Enter directory and output file name
       comment Enter directory where soundfiles are kept:
        ...
        sentence outFile formant-values-auto-v8median.txt
...
for ifile to numberOfFiles
    ...
    beginPause: "Debug"
    buttonChosen = endPause: "Continue", "Skip", 0
    #endPause: "Continue", 0
    ...
    if last_char$ = "l"
        if first_char$ = "l" or first_char$ = "m"
            ...
            while t < total_time and counter <= 0.8*total_time
                ...
                appendInfoLine: f1a
                appendInfoLine: f1b
                appendInfoLine:t[index]
                appendInfoLine:counter
        ...
        ## find sharpest change in intensity
            while counter <= 0.8*total_time
                ...
                appendInfoLine: "Int values:"
                appendInfoLine: inta
                appendInfoLine: intb
                appendInfoLine: t
                appendInfoLine: intDiff
                counter = counter + (change_in_counter)/2
            endwhile
        ...
        for i to windows_in_file
            median_intensity[i] = Get mean: start_time, end_time, "dB"
            start_time = end_time
        ...
        for i to windows_in_file
            median_f1[i] = Get quantile: 1, start_time, end_time, "Hertz", 0.50
            current_time[i] = (start_time + end_time) / 2

            if (median_f1[i] < 1000 and max_median_f1 < median_f1[i] and median_intensity[i] > 0.9*max_intensity)
                max_median_f1 = median_f1[i]
                t = current_time[i]
            endif
            start_time = end_time
            end_time = end_time + stDevWindLength
